...
public:
    ...
    void
    update_max(KeyType a_key, ValueT a_v){
      auto updatefn = [a_v](ValueT& num) { if(a_v.first > num.first) num = a_v; };
      IndexT hash_key = getIndex1D(a_key, m_shape);
      m_cmap.upsert(hash_key, updatefn, a_v);
    }
    ...
    template<typename ValT, typename IdT>void
    traverse(std::vector<KeyType>& keys, std::vector<ValT>& values, std::vector<IdT>& ids){
      ...
      for(auto it = lmap.begin(); it != lmap.end(); ++it, cnt++){
        keys[cnt] = getHighDimIndexVec(it->first, m_shape);
        values[cnt] = it->second.first;
        ids[cnt] = it->second.second;
      }
    }
