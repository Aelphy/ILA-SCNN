...
namespace tensorflow {
  ...
  template <typename IndexT, typename ValueT, typename ShapeT, typename T, typename IndiceT> void
  sparseCuboidConvKD(   const IndexT& in_ind, 
                        ...
                        const IndiceT dim,
                        std::vector<std::vector<IndiceT> >& output_keys,
                        std::vector<T>& output_values,
                        std::vector<IndiceT>& out_shape,
    ...
    std::vector<IndiceT> filter_offset(f_sh.dimension(0), 0);
    ...
    for(IndiceT i = 0; i < filter_offset.size(); ++i){
    ...
    LFMap<std::vector<IndiceT>, T> map(out_shape);
    auto map_ptr = &map;
...
#pragma omp parallel for firstprivate(in_ind_ptr, f_sh_ptr, approx_out_val_ptr, map_ptr)
      for(IndiceT i = 0; i < in_ind_ptr->dimension(0); ++i){
        std::vector<IndiceT> update_ids((*in_ind_ptr).dimension(1), 0);
...
#pragma omp parallel for firstprivate(in_ind_ptr, f_ind_ptr, in_vals_ptr, f_vals_ptr, in_sh_ptr, map_ptr, approx_out_val_ptr)
    for(IndiceT i = 0; i < (*in_ind_ptr).dimension(0); ++i){ //TODO: parallelize filtering
      //a) prepare filter to update output based on current value
      std::vector<std::pair<IndiceT,T> > filter_update;
      std::vector<IndiceT> update_ids((*in_ind_ptr).dimension(1), 0);
      for(IndiceT j = 0; j < (*f_ind_ptr).dimension(0); ++j){
        ...
        for(IndiceT k = id_f_depth, l = id_in_depth; k <= id_f_width; ++k, ++l){ //TODO: ugly coding style... prototype
          IndiceT out_plain_id = (IndiceT)((*in_ind_ptr)(i,l) - (*f_ind_ptr)(j,k) + filter_offset[l]);
          if(padding_type == 1){ //valid padding
    ...
    if(approx){
      IndiceT num_non_zero = 0;
      std::vector<IndiceT> look_up_ids((*approx_out_val_ptr).size(), 0);
      ...
      output_keys.assign(num_non_zero, std::vector<IndiceT>(in_ind.dimension(1)));
...
#pragma omp parallel for firstprivate(output_values_ptr, output_keys_ptr, in_ind_ptr, approx_out_val_ptr, look_up_ids_ptr)
      for(IndiceT i = 0; i < num_non_zero; ++i){
        (*output_values_ptr)[i] = (*approx_out_val_ptr)[(*look_up_ids_ptr)[i]];
        IndiceT data_id = (*look_up_ids_ptr)[i] % (*in_ind_ptr).dimension(0);
        IndiceT data_channel = ((*look_up_ids_ptr)[i] - data_id) / (*in_ind_ptr).dimension(0);
        ...
        for(IndiceT j = id_in_depth; j  <= id_in_width; ++j){
  ...
  template <typename IndexT, typename FIndexT, typename ValueT, typename KeyT, typename DataT, typename IndiceT>
  class ConvKDHelper {
  public:
    ConvKDHelper( const IndexT* data_index, 
                  ...
                  std::vector<IndiceT> filter_offset,
                  IndiceT dimension = 3)
  ...
  protected:
    ...
    int m_padding_type;
    IndiceT m_dimension;
    std::vector<IndiceT> m_filter_offset;
    std::map<KeyT, DataT> m_map;
  };

  //voting sheme convolution (see vote3d)
  template <typename IndexT, typename ValueT, typename ShapeT, typename T, typename OutT, typename IndiceT> void
  sparseCuboidConvKDBackpropV2(   const IndexT& in_ind, 
    ...
    //preparation: find center of filter
    std::vector<IndiceT> filter_offset(f_sh.dimension(0), 0);
    std::vector<IndiceT> out_shape(in_sh.dimension(0), 0);
...
#pragma omp parallel for firstprivate(map_ptr, grad_ind_ptr, grad_val_ptr)
    for(IndiceT i = 0; i < (*grad_ind_ptr).dimension(0); ++i){
      std::vector<IndiceT> update_ids((*grad_ind_ptr).dimension(1), 0);
...
#pragma omp parallel for firstprivate(input_bp_ptr)
    for(IndiceT i = 0; i < input_bp_ptr->size(); ++i){
      (*input_bp_ptr)[i] = 0;
    }
#pragma omp parallel for firstprivate(filter_bp_ptr)
    for(IndiceT i = 0; i < filter_bp_ptr->size(); ++i){
      (*filter_bp_ptr)[i] = 0;
    }
