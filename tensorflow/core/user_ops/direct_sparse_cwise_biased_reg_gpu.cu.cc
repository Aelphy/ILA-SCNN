...
template <typename dtype, typename itype, int data_dimension, int power> __global__ void  __launch_bounds__(MAX_1024_THREADS_PER_BLOCK)
biased_cwise_reg_bp(CudaLaunchConfig config, const itype* __restrict__ f_id_ptr, const dtype* __restrict__ f_val_ptr, 
  const itype* __restrict__ f_shape_ptr /*[batch, dim1, ..., dimx, channel_nr]*/, const int* __restrict__ f_mapping_ptr,
  const dtype* __restrict__ bias, const dtype* __restrict__ scale, const dtype* __restrict__ grads, dtype* backprops)
{
  itype id_kd[data_dimension];
  CUDA_1D_KERNEL_LOOP(x, config.virtual_thread_count){
    if (x < 0) {  //x might overflow when testing extreme case
    ...
    index_KDto1D_<itype, data_dimension>(&id_kd[0], f_shape_ptr, &in_id);
    auto out_channel = id_kd[data_dimension - 1];
    auto out_v = scale[0] * (f_val_ptr[x] + bias[out_channel]) * grads[0]; //TODO: power as template parameter (L1, L2)
    backprops[x] = out_v;
  }
}

template <typename DeviceT, typename T, typename IndiceT, int data_dimension, int power>
void DirectSparseCwiseBiasedRegBpFunctor<DeviceT, T, IndiceT, data_dimension, power>::operator()(OpKernelContext* context) const {
  const Tensor *f_indices, *f_values, *f_shape, *f_block_ptr_t, *bias, *scale, *gradients;
  OP_REQUIRES_OK(context, context->input("f_indices", &f_indices));
  OP_REQUIRES_OK(context, context->input("f_values", &f_values));
  OP_REQUIRES_OK(context, context->input("f_shape", &f_shape));
  OP_REQUIRES_OK(context, context->input("f_channel_mapping", &f_block_ptr_t));
  ...
  cudaMemcpy(&out_channel_count, f_sh.data() + data_dimension - 1, sizeof(IndiceT), cudaMemcpyDeviceToHost);
	...
	Tensor *backprops;
	TensorShape out_bp_shape = {(IndiceT) f_val.dimension(0)};
	OP_REQUIRES_OK(context, context->allocate_output("backprops", out_bp_shape, &backprops));
  auto bp = backprops->flat<T>();
  cudaMemset(bp.data(), 0, bp.dimension(0) * sizeof(T));
  if(filter_weight_count > 0){
    CudaLaunchConfig config = GetCudaLaunchConfig(filter_weight_count, d);
    biased_cwise_reg_bp<T, IndiceT, data_dimension, power><<<config.block_count, config.thread_per_block, 0, d.stream()>>>(config, f_ind.data(), f_val.data(), f_sh.data(), f_mapping.data(), 
        b.data(), s.data(), grads.data(), bp.data());
  }
}
...
#define INIT_GPU_TYPE(type, indice_type, dim, power) \
 ...
 template struct functor::DirectSparseCwiseBiasedRegBpFunctor<GPUDevice, type, indice_type, dim, power>;
