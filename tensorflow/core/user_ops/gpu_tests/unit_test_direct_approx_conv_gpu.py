...
def verifyValues(tensor_in_sizes, filter_in_sizes, stride, rho_data = 1, rho_filter = 1, padding = 'SAME', dim = 5, max_density = 1, num_trials = 3, filter_type="K-RELU", test_type = ""):
  ...
  ts = 0
  with tf.device("/gpu:0"):
    approx_scskconv = sc_module.direct_sparse_approx_conv_kd(pd.out_indices, pd.out_values, pd.out_shape, pd.out_block_channel_mapping, pf.out_indices, pf.out_values, pf.out_shape, pf.out_channel_mapping, pd.out_indices, pd.out_shape, strides, padding, dim, max_density, filter_type);
  with tf.Session(config=config) as sess:
    t6 = time.time()
  ...
  [bp_ind, pd_bp_val, bp_sh] = sp.createRandomSparseTensor(1, [len(sv3)], 1, 9)
  d3_ = sp.sparse1d_to_dense(pd.out_indices, pd_bp_val, pd.out_shape, pd.out_block_channel_mapping[-1])
  ...
  with tf.Session(config=config) as sess:
    with tf.device("/gpu:0"):
      fbp = sc_module.direct_sparse_approx_conv_kd_backprop(pd.out_indices, pd.out_values, pd.out_shape, pd.out_block_channel_mapping, pf.out_indices, pf.out_values, pf.out_shape, pf.out_channel_mapping, pd.out_indices, pd.out_values, pd.out_shape, pd.out_block_channel_mapping, pd_bp_val, strides, padding, dim, max_density)
  ...
  value3 = sp.sparse1d_to_dense(pd.out_indices, sv3, pd.out_shape, pd.out_block_channel_mapping[-1])
  #print("sparse result", value3)
...
ret_value = verifyValues(
  ...
  filter_in_sizes=[filter_res, filter_res, filter_res, channel_count, channel_count], #[depth, height, width, in_channels, out_channels] 
  stride=1,
