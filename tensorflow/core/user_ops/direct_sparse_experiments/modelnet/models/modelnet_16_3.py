...
def get_model(sparse_data, train_labels, is_training, tensor_in_sizes, num_classes = 10, scope = "mn16-", initializer = None, regularizer = None):
  ...
  net = ld.create_sparse_conv_layer(net, [3,3,3,8,16], strides, padding, dim, 2 * d1, "K-RELU", name = scope + "sc4", initializer=initializer)
  net = ld.create_sparse_conv_layer(net, [3,3,3,16,16], strides, padding, dim, 2 * d1, "K-RELU", name = scope + "sc5", initializer=initializer)
  net = ld.create_sparse_conv_layer(net, [3,3,3,16,16], strides, padding, dim, 2 * d1, "K-ABS", name = scope + "sc6", initializer=initializer)
