...
def get_model(sparse_data, train_labels, is_training, tensor_in_sizes, num_classes = 10, scope = "mn16-", initializer = None, regularizer = None):
  ...
  ops = [None]*6
  d1 = 0.06
  net, tensor_in_sizes = ld.create_sparse_conv_layer(sd_converted, [3,3,3,1,12], tensor_in_sizes, strides, padding, dim, d1, "K-RELU", name = scope + "sc1", initializer=initializer)
  net, tensor_in_sizes = ld.create_sparse_conv_layer(net, [3,3,3,12,12], tensor_in_sizes, strides, padding, dim, d1, "K-RELU", name = scope + "sc2", initializer=initializer)
  net, tensor_in_sizes = ld.create_sparse_conv_layer(net, [3,3,3,12,12], tensor_in_sizes, strides, padding, dim, d1, "K-RELU", name = scope + "sc3", initializer=initializer)
  net, tensor_in_sizes = ld.create_sparse_pooling_layer(net, pooling_sizes, tensor_in_sizes, dim, 6 * d1)
  d2 = 0.12
  net, tensor_in_sizes = ld.create_sparse_conv_layer(net, [3,3,3,12,24], tensor_in_sizes, strides, padding, dim, d2, "K-RELU", name = scope + "sc4", initializer=initializer)
  net, tensor_in_sizes = ld.create_sparse_conv_layer(net, [3,3,3,24,24], tensor_in_sizes, strides, padding, dim, d2, "K-RELU", name = scope + "sc5", initializer=initializer)
  net, tensor_in_sizes = ld.create_sparse_conv_layer(net, [3,3,3,24,24], tensor_in_sizes, strides, padding, dim, d2, "K-ABS", name = scope + "sc6", initializer=initializer)
  sd = ld.create_direct_sparse_to_dense(net, dim)
  sd_flat = tf.reshape(sd, [batch_size, total_size * 3])
