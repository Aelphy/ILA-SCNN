...
def get_model(sparse_data, train_labels, is_training, tensor_in_sizes, num_classes = 10, scope = "mn16-", initializer = None, regularizer = None):
  ...
  ops = [None]*6
  d1 = 0.06
  net, tensor_in_sizes = ld.create_sparse_conv_layer(sd_converted, [9,9,9,1,8], tensor_in_sizes, strides, padding, dim, d1, "K-RELU", name = scope + "sc1", initializer=initializer)
  net, tensor_in_sizes = ld.create_sparse_conv_layer(net, [9,9,9,8,8], tensor_in_sizes, strides, padding, dim, d1, "K-RELU", name = scope + "sc2", initializer=initializer)
  net, tensor_in_sizes = ld.create_sparse_conv_layer(net, [9,9,9,8,8], tensor_in_sizes, strides, padding, dim, d1, "K-RELU", name = scope + "sc3", initializer=initializer)
  net, tensor_in_sizes = ld.create_sparse_pooling_layer(net, pooling_sizes, tensor_in_sizes, dim, 8 * d1)
  d2 = 0.12
  net, tensor_in_sizes = ld.create_sparse_conv_layer(net, [9,9,9,8,16], tensor_in_sizes, strides, padding, dim, d2, "K-RELU", name = scope + "sc4", initializer=initializer)
  net, tensor_in_sizes = ld.create_sparse_conv_layer(net, [9,9,9,16,16], tensor_in_sizes, strides, padding, dim, d2, "K-RELU", name = scope + "sc5", initializer=initializer)
  net, tensor_in_sizes = ld.create_sparse_conv_layer(net, [9,9,9,16,16], tensor_in_sizes, strides, padding, dim, d2, "K-ABS", name = scope + "sc6", initializer=initializer)
