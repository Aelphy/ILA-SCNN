...
def get_model(sparse_data, train_labels, is_training, tensor_in_sizes, num_classes = 10, scope = "mn16-", initializer = None, regularizer = None):
  ...
  d2 = 0.12
  net, tensor_in_sizes = ld.create_sparse_conv_layer(net, [3,3,3,16,32], tensor_in_sizes, strides, padding, dim, d2, "K-RELU", name = scope + "sc4", initializer=initializer)
  net, tensor_in_sizes = ld.create_sparse_conv_layer(net, [3,3,3,32,32], tensor_in_sizes, strides, padding, dim, d2, "K-RELU", name = scope + "sc5", initializer=initializer)
  net, tensor_in_sizes = ld.create_sparse_conv_layer(net, [3,3,3,32,32], tensor_in_sizes, strides, padding, dim, d2, "K-ABS", name = scope + "sc6", initializer=initializer)
  sd = ld.create_direct_sparse_to_dense(net, dim)
  sd_flat = tf.reshape(sd, [batch_size, total_size * 4])
